//
//  File.swift
//  
//
//  Created by Vanson Leung on 13/9/2023.
//

import Foundation

public struct KCItunesParamCountry {
    public let abbr: String?
    public let chinese_sc: String?
    public let chinese_tc: String?
    public let code: String?
    public let english: String?
    public let spell: String?
    public let state: String?
    public let french: String?
    public let italian: String?
    public let spanish: String?
    public let japanese: String?
    public let russian: String?
    public let germen: String?
}

public struct KCItunesParamCountryManager {
    public static let shared = KCItunesParamCountryManager()
    
    public var CountryListArray : [KCItunesParamCountry]
    public var CountryDictMappingByAbbr : [String: KCItunesParamCountry]
    
    init() {
        CountryListArray = CountryListArrayRawDataSource.map({ dict in
            KCItunesParamCountry(
                abbr: dict["abbr"],
                chinese_sc: dict["chinese_sc"],
                chinese_tc: dict["chinese_tc"],
                code: dict["code"],
                english: dict["english"],
                spell: dict["spell"],
                state: dict["state"],
                french: dict["french"],
                italian: dict["italian"],
                spanish: dict["spanish"],
                japanese: dict["japanese"],
                russian: dict["russian"],
                germen: dict["germen"]
            )
        })
        
        var c_dict_abbr : [String: KCItunesParamCountry] = [:]
        
        for it in CountryListArray {
            if let abbr = it.abbr {
                c_dict_abbr[abbr] = it
            }
        }
        
        CountryDictMappingByAbbr = c_dict_abbr
    }
    
    
    public var CountryListArrayRawDataSource = [
        [
          "abbr": "AF",
          "chinese_sc": "阿富汗",
          "chinese_tc": "阿富汗",
          "code": "93",
          "english": "Afghanistan",
          "spell": "afuhan",
          "state": "亚洲",
          "french": "Afghanistan",
          "italian": "Afghanistan",
          "spanish": "Afganistán",
          "japanese": "アフガニスタン",
          "russian": "Афганистан",
          "germen": "Afghanistan"
        ],
        [
          "abbr": "AL",
          "chinese_sc": "阿尔巴尼亚",
          "chinese_tc": "阿爾巴尼亞",
          "code": "355",
          "english": "Albania",
          "spell": "aerbaniya",
          "state": "亚洲",
          "french": "Albanie",
          "italian": "Albania",
          "spanish": "Albania",
          "japanese": "アルバニア",
          "russian": "Албания",
          "germen": "Albanien"
        ],
        [
          "abbr": "DZ",
          "chinese_sc": "阿尔及利亚",
          "chinese_tc": "阿爾及利亞",
          "code": "213",
          "english": "Algeria",
          "spell": "aerjiliya",
          "state": "非洲",
          "french": "Algérie",
          "italian": "Algeria",
          "spanish": "Argelia",
          "japanese": "アルジェリア",
          "russian": "Алжир",
          "germen": "Algerien"
        ],
        [
          "abbr": "AO",
          "chinese_sc": "安哥拉",
          "chinese_tc": "安哥拉",
          "code": "244",
          "english": "Angola",
          "spell": "angela",
          "state": "非洲",
          "french": "Angola",
          "italian": "Angola",
          "spanish": "Angola",
          "japanese": "アンゴラ",
          "russian": "Ангола",
          "germen": "Angola"
        ],
        [
          "abbr": "AR",
          "chinese_sc": "阿根廷",
          "chinese_tc": "阿根廷",
          "code": "54",
          "english": "Argentina",
          "spell": "agenting",
          "state": "南美洲",
          "french": "Argentine",
          "italian": "Argentina",
          "spanish": "Argentina",
          "japanese": "アルゼンチン",
          "russian": "Аргентина",
          "germen": "Argentinien"
        ],
        [
          "abbr": "AM",
          "chinese_sc": "亚美尼亚",
          "chinese_tc": "亞美尼亞",
          "code": "374",
          "english": "Armenia",
          "spell": "yameiniya",
          "french": "Arménie",
          "italian": "Armenia",
          "spanish": "Armenia",
          "japanese": "アルメニア",
          "russian": "Армения",
          "germen": "Armenien"
        ],
        [
          "abbr": "AU",
          "chinese_sc": "澳大利亚",
          "chinese_tc": "澳大利亞",
          "code": "61",
          "english": "Australia",
          "spell": "aodaliya",
          "state": "大洋洲",
          "french": "Îles Cocos",
          "italian": "Isole Cocos e Keeling",
          "spanish": "Islas Cocos o Islas Keeling",
          "japanese": "ココス（キーリング）諸島",
          "russian": "Австралия",
          "germen": "Kokosinseln"
        ],
        [
          "abbr": "AT",
          "chinese_sc": "奥地利",
          "chinese_tc": "奧地利",
          "code": "43",
          "english": "Austria",
          "spell": "aodili",
          "state": "欧洲",
          "french": "Autriche",
          "italian": "Austria",
          "spanish": "Austria",
          "japanese": "オーストリア",
          "russian": "Австрия",
          "germen": "Österreich"
        ],
        [
          "abbr": "AZ",
          "chinese_sc": "阿塞拜疆",
          "chinese_tc": "阿塞拜疆",
          "code": "994",
          "english": "Azerbaijan",
          "spell": "asaibaijiang",
          "state": "亚洲",
          "french": "Azerbaïdjan",
          "italian": "Azerbaijan",
          "spanish": "Azerbaiyán",
          "japanese": "アゼルバイジャン",
          "russian": "Азербайджан",
          "germen": "Aserbaidschan"
        ],
        [
          "abbr": "BH",
          "chinese_sc": "巴林",
          "chinese_tc": "巴林",
          "code": "973",
          "english": "Bahrain",
          "spell": "balin",
          "state": "亚洲",
          "french": "Bahreïn",
          "italian": "Bahrein",
          "spanish": "Bahrein",
          "japanese": "バーレーン",
          "russian": "Бахрейн",
          "germen": "Bahrain"
        ],
        [
          "abbr": "BD",
          "chinese_sc": "孟加拉国",
          "chinese_tc": "孟加拉國",
          "code": "880",
          "english": "Bangladesh",
          "spell": "mengjialaguo",
          "state": "亚洲",
          "french": "Bangladesh",
          "italian": "Bangladesh",
          "spanish": "Bangladesh",
          "japanese": "バングラデシュ",
          "russian": "Бангладеш",
          "germen": "Bangladesch"
        ],
        [
          "abbr": "BY",
          "chinese_sc": "白俄罗斯",
          "chinese_tc": "白俄羅斯",
          "code": "375",
          "english": "Belarus",
          "spell": "baieluosi",
          "french": "Biélorussie",
          "italian": "Bielorussia",
          "spanish": "Bielorrusia",
          "japanese": "ベラルーシ",
          "russian": "Белоруссия",
          "germen": "Weißrussland"
        ],
        [
          "abbr": "BE",
          "chinese_sc": "比利时",
          "chinese_tc": "比利時",
          "code": "32",
          "english": "Belgium",
          "spell": "bilishi",
          "state": "欧洲",
          "french": "Belgique",
          "italian": "Belgio",
          "spanish": "Bélgica",
          "japanese": "ベルギー",
          "russian": "Бельгия",
          "germen": "Belgien"
        ],
        [
          "abbr": "BZ",
          "chinese_sc": "伯利兹",
          "chinese_tc": "伯利茲",
          "code": "501",
          "english": "Belize",
          "spell": "bolizi",
          "french": "Belize",
          "italian": "Belize",
          "spanish": "Belice",
          "japanese": "ベリーズ",
          "russian": "Белиз",
          "germen": "Belize"
        ],
        [
          "abbr": "BJ",
          "chinese_sc": "贝宁",
          "chinese_tc": "貝寧",
          "code": "229",
          "english": "Benin",
          "spell": "beining",
          "state": "非洲",
          "french": "Bénin",
          "italian": "Benin",
          "spanish": "Benín",
          "japanese": "ベナン",
          "russian": "Бенин",
          "germen": "Benin"
        ],
        [
          "abbr": "BT",
          "chinese_sc": "不丹",
          "chinese_tc": "不丹",
          "code": "975",
          "english": "Bhutan",
          "spell": "budan",
          "state": "亚洲",
          "french": "Bhoutan",
          "italian": "Bhutan",
          "spanish": "Bután",
          "japanese": "ブータン",
          "russian": "Бутан",
          "germen": "Bhutan"
        ],
        [
          "abbr": "BO",
          "chinese_sc": "玻利维亚",
          "chinese_tc": "玻利維亞",
          "code": "591",
          "english": "Bolivia",
          "spell": "boliweiya",
          "state": "南美洲",
          "french": "Bolivie",
          "italian": "Bolivia",
          "spanish": "Bolivia",
          "japanese": "ボリビア多民族国",
          "russian": "Боливия",
          "germen": "Bolivien"
        ],
        [
          "abbr": "BA",
          "chinese_sc": "波斯尼亚和黑塞哥维那",
          "chinese_tc": "波斯尼亞和黑塞哥維那",
          "code": "387",
          "english": "Bosnia and Herzegovina",
          "spell": "bosiniyaheheisaigeweinei",
          "french": "Bosnie-Herzégovine",
          "italian": "Bosnia ed Erzegovina",
          "spanish": "Bosnia y Herzegovina",
          "japanese": "ボスニア・ヘルツェゴビナ",
          "russian": "Босния и Герцеговина",
          "germen": "Bosnien und Herzegowina"
        ],
        [
          "abbr": "BW",
          "chinese_sc": "博茨瓦纳",
          "chinese_tc": "博茨瓦納",
          "code": "267",
          "english": "Botswana",
          "spell": "bociwana",
          "state": "非洲",
          "french": "Botswana",
          "italian": "Botswana",
          "spanish": "Botswana",
          "japanese": "ボツワナ",
          "russian": "Ботсвана",
          "germen": "Botswana"
        ],
        [
          "abbr": "BR",
          "chinese_sc": "巴西",
          "chinese_tc": "巴西",
          "code": "55",
          "english": "Brazil",
          "spell": "baxi",
          "state": "南美洲",
          "french": "Brésil",
          "italian": "Brasile",
          "spanish": "Brasil",
          "japanese": "ブラジル",
          "russian": "Бразилия",
          "germen": "Brasilien"
        ],
        [
          "abbr": "VG",
          "chinese_sc": "英属维京群岛",
          "chinese_tc": "英屬維京群島",
          "code": "1284",
          "english": "British Virgin Islands",
          "spell": "yingshuweijingqundao",
          "french": "Îles Vierges britanniques",
          "italian": "Isole Vergini Britanniche",
          "spanish": "Islas Vírgenes del Reino Unido",
          "japanese": "イギリス領ヴァージン諸島",
          "russian": "Британские Виргинские острова",
          "germen": "Britische Jungferninseln"
        ],
        [
          "abbr": "BN",
          "chinese_sc": "文莱",
          "chinese_tc": "文萊",
          "code": "673",
          "english": "Brunei",
          "spell": "wenlai",
          "state": "亚洲",
          "french": "Brunei",
          "italian": "Brunei",
          "spanish": "Brunei",
          "japanese": "ブルネイ・ダルサラーム",
          "russian": "Bulgaria",
          "germen": "Brunei"
        ],
        [
          "abbr": "BG",
          "chinese_sc": "保加利亚",
          "chinese_tc": "保加利亞",
          "code": "359",
          "english": "Bulgaria",
          "spell": "baojialiya",
          "state": "欧洲",
          "french": "Bulgarie",
          "italian": "Bulgaria",
          "spanish": "Bulgaria",
          "japanese": "ブルガリア",
          "russian": "Болгария",
          "germen": "Bulgarien"
        ],
        [
          "abbr": "BF",
          "chinese_sc": "布基纳法索",
          "chinese_tc": "布基納法索",
          "code": "226",
          "english": "Burkina-faso",
          "spell": "bujinafasuo",
          "french": "Burkina Faso",
          "italian": "Burkina Faso",
          "spanish": "Burkina Faso",
          "japanese": "ブルキナファソ",
          "russian": "Буркина-Фасо",
          "germen": "Burkina Faso"
        ],
        [
          "abbr": "BI",
          "chinese_sc": "布隆迪",
          "chinese_tc": "布隆迪",
          "code": "257",
          "english": "Burundi",
          "spell": "bulongdi",
          "state": "非洲",
          "french": "Burundi",
          "italian": "Burundi",
          "spanish": "Burundi",
          "japanese": "ブルンジ",
          "russian": "Бурунди",
          "germen": "Burundi"
        ],
        [
          "abbr": "KH",
          "chinese_sc": "柬埔寨",
          "chinese_tc": "柬埔寨",
          "code": "855",
          "english": "Cambodia",
          "spell": "jianpuzhai",
          "state": "亚洲",
          "french": "Cambodge",
          "italian": "Cambogia",
          "spanish": "Camboya",
          "japanese": "カンボジア",
          "russian": "Камбоджа",
          "germen": "Kambodscha"
        ],
        [
          "abbr": "CM",
          "chinese_sc": "喀麦隆",
          "chinese_tc": "喀麥隆",
          "code": "237",
          "english": "Cameroon",
          "spell": "kamailong",
          "state": "非洲",
          "french": "Cameroun",
          "italian": "Camerun",
          "spanish": "Camerún",
          "japanese": "カメルーン",
          "russian": "Камерун",
          "germen": "Kamerun"
        ],
        [
          "abbr": "CA",
          "chinese_sc": "加拿大",
          "chinese_tc": "加拿大",
          "code": "1",
          "english": "Canada",
          "spell": "jianada",
          "state": "北美洲",
          "french": "États-Unis",
          "japanese": "アメリカ合衆国",
          "russian": "Канада",
          "germen": "Vereinigte Staaten von Amerika"
        ],
        [
          "abbr": "CV",
          "chinese_sc": "佛得角",
          "chinese_tc": "佛得角",
          "code": "238",
          "english": "Cape Verde",
          "spell": "fodejiao",
          "state": "非洲",
          "french": "Cap Vert",
          "italian": "Capo Verde",
          "spanish": "Cabo Verde",
          "japanese": "カーボベルデ",
          "russian": "Кабо-Верде",
          "germen": "Kap Verde"
        ],
        [
          "abbr": "KY",
          "chinese_sc": "开曼群岛",
          "chinese_tc": "開曼群島",
          "code": "1345",
          "english": "Cayman Islands",
          "spell": "kaimanqundao",
          "french": "Îles Caïmans",
          "italian": "Isole Cayman",
          "spanish": "Islas Caimán",
          "japanese": "ケイマン諸島",
          "russian": "Каймановы Острова",
          "germen": "Kaimaninseln"
        ],
        [
          "abbr": "CF",
          "chinese_sc": "中非共和国",
          "chinese_tc": "中非共和國",
          "code": "236",
          "english": "Central African Republic",
          "spell": "zhongfeigongheguo",
          "state": "非洲",
          "french": "République centrafricaine",
          "italian": "Repubblica Centrafricana",
          "spanish": "República Centroafricana",
          "japanese": "中央アフリカ共和国",
          "russian": "Центрально-Африканская Республика",
          "germen": "Zentralafrikanische Republik"
        ],
        [
          "abbr": "TD",
          "chinese_sc": "乍得",
          "chinese_tc": "乍得",
          "code": "235",
          "english": "Chad",
          "spell": "zhade",
          "french": "Tchad",
          "italian": "Ciad",
          "spanish": "Chad",
          "japanese": "チャド",
          "russian": "Чад",
          "germen": "Tschad"
        ],
        [
          "abbr": "CL",
          "chinese_sc": "智利",
          "chinese_tc": "智利",
          "code": "56",
          "english": "Chile",
          "spell": "zhili",
          "state": "南美洲",
          "french": "Chili",
          "italian": "Cile",
          "spanish": "Chile",
          "japanese": "チリ",
          "russian": "Чили",
          "germen": "Chile"
        ],
        [
          "abbr": "CN",
          "chinese_sc": "中国",
          "chinese_tc": "中國",
          "code": "86",
          "english": "China",
          "spell": "zhongguo",
          "state": "亚洲",
          "french": "Chine",
          "italian": "Cina",
          "spanish": "China",
          "japanese": "中国",
          "russian": "Китай",
          "germen": "China"
        ],
        [
          "abbr": "CO",
          "chinese_sc": "哥伦比亚",
          "chinese_tc": "哥倫比亞",
          "code": "57",
          "english": "Colombia",
          "spell": "gelunbiya",
          "state": "南美洲",
          "french": "Colombie",
          "italian": "Colombia",
          "spanish": "Colombia",
          "japanese": "コロンビア",
          "russian": "Колумбия",
          "germen": "Kolumbien"
        ],
        [
          "abbr": "KM",
          "chinese_sc": "科摩罗",
          "chinese_tc": "科摩羅",
          "code": "269",
          "english": "Comoros",
          "spell": "kemoluo",
          "french": "Comores",
          "italian": "Comore",
          "spanish": "Comoras",
          "japanese": "コモロ",
          "russian": "Коморские острова",
          "germen": "Union der Komoren"
        ],
        [
          "abbr": "CG",
          "chinese_sc": "刚果(布)",
          "chinese_tc": "剛果(布)",
          "code": "242",
          "english": "Congo - Brazzaville",
          "spell": "gangguo(bu)",
          "state": "非洲",
          "french": "Congo",
          "italian": "Congo",
          "spanish": "Congo",
          "japanese": "コンゴ共和国",
          "russian": "Конго - Браззавиль",
          "germen": "Kongo"
        ],
        [
          "abbr": "CD",
          "chinese_sc": "刚果(金)",
          "chinese_tc": "剛果(金)",
          "code": "243",
          "english": "Congo - Kinshasa",
          "spell": "gangguo(jin)",
          "french": "Congo (Rép. dém.)",
          "italian": "Congo (Rep. Dem.)",
          "spanish": "Congo (Rep. Dem.)",
          "japanese": "コンゴ民主共和国",
          "russian": "Конго - Киншаса",
          "germen": "Kongo (Dem. Rep.)"
        ],
        [
          "abbr": "CR",
          "chinese_sc": "哥斯达黎加",
          "chinese_tc": "哥斯達黎加",
          "code": "506",
          "english": "Costa Rica",
          "spell": "gesidalijia",
          "state": "北美洲",
          "french": "Costa Rica",
          "italian": "Costa Rica",
          "spanish": "Costa Rica",
          "japanese": "コスタリカ",
          "russian": "Коста-Рика",
          "germen": "Costa Rica"
        ],
        [
          "abbr": "HR",
          "chinese_sc": "克罗地亚",
          "chinese_tc": "克羅地亞",
          "code": "385",
          "english": "Croatia",
          "spell": "keluodiya",
          "state": "欧洲",
          "french": "Croatie",
          "italian": "Croazia",
          "spanish": "Croacia",
          "japanese": "クロアチア",
          "russian": "Хорватия",
          "germen": "Kroatien"
        ],
        [
          "abbr": "CY",
          "chinese_sc": "塞浦路斯",
          "chinese_tc": "塞浦路斯",
          "code": "357",
          "english": "Cyprus",
          "spell": "saipulusi",
          "state": "亚洲",
          "french": "Chypre",
          "italian": "Cipro",
          "spanish": "Chipre",
          "japanese": "キプロス",
          "russian": "Кипр",
          "germen": "Zypern"
        ],
        [
          "abbr": "CZ",
          "chinese_sc": "捷克共和国",
          "chinese_tc": "捷克共和國",
          "code": "420",
          "english": "Czech Republic",
          "spell": "jiekegongheguo",
          "state": "欧洲",
          "french": "République tchèque",
          "italian": "Repubblica Ceca",
          "spanish": "República Checa",
          "japanese": "チェコ",
          "russian": "Чехия",
          "germen": "Tschechische Republik"
        ],
        [
          "abbr": "DK",
          "chinese_sc": "丹麦",
          "chinese_tc": "丹麥",
          "code": "45",
          "english": "Denmark",
          "spell": "danmai",
          "state": "欧洲",
          "french": "Danemark",
          "italian": "Danimarca",
          "spanish": "Dinamarca",
          "japanese": "デンマーク",
          "russian": "Дания",
          "germen": "Dänemark"
        ],
        [
          "abbr": "DJ",
          "chinese_sc": "吉布提",
          "chinese_tc": "吉布提",
          "code": "253",
          "english": "Djibouti",
          "spell": "jibuti",
          "french": "Djibouti",
          "italian": "Gibuti",
          "spanish": "Yibuti",
          "japanese": "ジブチ",
          "russian": "Джибути",
          "germen": "Dschibuti"
        ],
        [
          "abbr": "DO",
          "chinese_sc": "多米尼加共和国",
          "chinese_tc": "多米尼加共和國",
          "code": "1809",
          "english": "Dominican Republic",
          "spell": "duominijiagongheguo",
          "french": "République dominicaine",
          "italian": "Repubblica Dominicana",
          "spanish": "República Dominicana",
          "japanese": "ドミニカ共和国",
          "russian": "Доминиканская Республика",
          "germen": "Dominikanische Republik"
        ],
        [
          "abbr": "EC",
          "chinese_sc": "厄瓜多尔",
          "chinese_tc": "厄瓜多爾",
          "code": "593",
          "english": "Ecuador",
          "spell": "eguaduoer",
          "state": "南美洲",
          "french": "Équateur",
          "italian": "Ecuador",
          "spanish": "Ecuador",
          "japanese": "エクアドル",
          "russian": "Эквадор",
          "germen": "Ecuador"
        ],
        [
          "abbr": "EG",
          "chinese_sc": "埃及",
          "chinese_tc": "埃及",
          "code": "20",
          "english": "Egypt",
          "spell": "aiji",
          "state": "非洲",
          "french": "Égypte",
          "italian": "Egitto",
          "spanish": "Egipto",
          "japanese": "エジプト",
          "russian": "Египет",
          "germen": "Ägypten"
        ],
        [
          "abbr": "SV",
          "chinese_sc": "萨尔瓦多",
          "chinese_tc": "薩爾瓦多",
          "code": "503",
          "english": "EI Salvador",
          "spell": "saerwaduo",
          "french": "Salvador",
          "italian": "El Salvador",
          "spanish": "El Salvador",
          "japanese": "エルサルバドル",
          "russian": "EI Сальвадор",
          "germen": "El Salvador"
        ],
        [
          "abbr": "GQ",
          "chinese_sc": "赤道几内亚",
          "chinese_tc": "赤道幾內亞",
          "code": "240",
          "english": "Equatorial Guinea",
          "spell": "chidaojineiya",
          "state": "非洲",
          "french": "Guinée-Équatoriale",
          "italian": "Guinea Equatoriale",
          "spanish": "Guinea Ecuatorial",
          "japanese": "赤道ギニア",
          "russian": "Экваториальная Гвинея",
          "germen": "Äquatorial-Guinea"
        ],
        [
          "abbr": "ER",
          "chinese_sc": "厄立特里亚",
          "chinese_tc": "厄立特里亞",
          "code": "291",
          "english": "Eritrea",
          "spell": "eliteliya",
          "french": "Érythrée",
          "italian": "Eritrea",
          "spanish": "Eritrea",
          "japanese": "エリトリア",
          "russian": "Эритрея",
          "germen": "Eritrea"
        ],
        [
          "abbr": "EE",
          "chinese_sc": "爱沙尼亚",
          "chinese_tc": "愛沙尼亞",
          "code": "372",
          "english": "Estonia",
          "spell": "aishaniya",
          "state": "欧洲",
          "french": "Estonie",
          "italian": "Estonia",
          "spanish": "Estonia",
          "japanese": "エストニア",
          "russian": "Эстония",
          "germen": "Estland"
        ],
        [
          "abbr": "ET",
          "chinese_sc": "埃塞俄比亚",
          "chinese_tc": "埃塞俄比亞",
          "code": "251",
          "english": "Ethiopia",
          "spell": "aisaiebiya",
          "state": "非洲",
          "french": "Éthiopie",
          "italian": "Etiopia",
          "spanish": "Etiopía",
          "japanese": "エチオピア",
          "russian": "Эфиопия",
          "germen": "Äthiopien"
        ],
        [
          "abbr": "FJ",
          "chinese_sc": "斐济",
          "chinese_tc": "斐濟",
          "code": "679",
          "english": "Fiji",
          "spell": "feiji",
          "state": "大洋洲",
          "french": "Fidji",
          "italian": "Figi",
          "spanish": "Fiyi",
          "japanese": "フィジー",
          "russian": "Фиджи",
          "germen": "Fidschi"
        ],
        [
          "abbr": "FI",
          "chinese_sc": "芬兰",
          "chinese_tc": "芬蘭",
          "code": "358",
          "english": "Finland",
          "spell": "fenlan",
          "state": "欧洲",
          "french": "Finlande",
          "italian": "Finlandia",
          "spanish": "Finlandia",
          "japanese": "フィンランド",
          "russian": "Финляндия",
          "germen": "Finnland"
        ],
        [
          "abbr": "FR",
          "chinese_sc": "法国",
          "chinese_tc": "法國",
          "code": "33",
          "english": "France",
          "spell": "faguo",
          "state": "欧洲",
          "french": "France",
          "italian": "Francia",
          "spanish": "Francia",
          "japanese": "フランス",
          "russian": "Франция",
          "germen": "Frankreich"
        ],
        [
          "abbr": "GA",
          "chinese_sc": "加蓬",
          "chinese_tc": "加蓬",
          "code": "241",
          "english": "Gabon",
          "spell": "jiapeng",
          "state": "非洲",
          "french": "Gabon",
          "italian": "Gabon",
          "spanish": "Gabón",
          "japanese": "ガボン",
          "russian": "Габон",
          "germen": "Gabun"
        ],
        [
          "abbr": "GM",
          "chinese_sc": "冈比亚",
          "chinese_tc": "岡比亞",
          "code": "220",
          "english": "Gambia",
          "spell": "gangbiya",
          "state": "非洲",
          "french": "Gambie",
          "italian": "Gambia",
          "spanish": "Gambia",
          "japanese": "ガンビア",
          "russian": "Гамбия",
          "germen": "Gambia"
        ],
        [
          "abbr": "GE",
          "chinese_sc": "格鲁吉亚",
          "chinese_tc": "格魯吉亞",
          "code": "995",
          "english": "Georgia",
          "spell": "gelujiya",
          "french": "Géorgie",
          "italian": "Georgia",
          "spanish": "Georgia",
          "japanese": "グルジア",
          "russian": "Грузия",
          "germen": "Georgien"
        ],
        [
          "abbr": "DE",
          "chinese_sc": "德国",
          "chinese_tc": "德國",
          "code": "49",
          "english": "Germany",
          "spell": "deguo",
          "state": "欧洲",
          "french": "Allemagne",
          "italian": "Germania",
          "spanish": "Alemania",
          "japanese": "ドイツ",
          "russian": "Германия",
          "germen": "Deutschland"
        ],
        [
          "abbr": "GH",
          "chinese_sc": "加纳",
          "chinese_tc": "加納",
          "code": "233",
          "english": "Ghana",
          "spell": "jiana",
          "state": "非洲",
          "french": "Ghana",
          "italian": "Ghana",
          "spanish": "Ghana",
          "japanese": "ガーナ",
          "russian": "Гана",
          "germen": "Ghana"
        ],
        [
          "abbr": "GR",
          "chinese_sc": "希腊",
          "chinese_tc": "希臘",
          "code": "30",
          "english": "Greece",
          "spell": "xila",
          "state": "欧洲",
          "french": "Grèce",
          "italian": "Grecia",
          "spanish": "Grecia",
          "japanese": "ギリシャ",
          "russian": "Греция",
          "germen": "Griechenland"
        ],
        [
          "abbr": "GL",
          "chinese_sc": "格陵兰",
          "chinese_tc": "格陵蘭",
          "code": "299",
          "english": "Greenland",
          "spell": "gelinglan",
          "french": "Groenland",
          "italian": "Groenlandia",
          "spanish": "Groenlandia",
          "japanese": "グリーンランド",
          "russian": "Гренландия",
          "germen": "Grönland"
        ],
        [
          "abbr": "GT",
          "chinese_sc": "危地马拉",
          "chinese_tc": "危地馬拉",
          "code": "502",
          "english": "Guatemala",
          "spell": "weidimala",
          "state": "北美洲",
          "french": "Guatemala",
          "italian": "Guatemala",
          "spanish": "Guatemala",
          "japanese": "グアテマラ",
          "russian": "Гватемала",
          "germen": "Guatemala"
        ],
        [
          "abbr": "GN",
          "chinese_sc": "几内亚",
          "chinese_tc": "幾內亞",
          "code": "224",
          "english": "Guinea",
          "spell": "jineiya",
          "state": "非洲",
          "french": "Guinée",
          "italian": "Guinea",
          "spanish": "Guinea",
          "japanese": "ギニア",
          "russian": "Гвинея",
          "germen": "Guinea"
        ],
        [
          "abbr": "GY",
          "chinese_sc": "圭亚那",
          "chinese_tc": "圭亞那",
          "code": "592",
          "english": "Guyana",
          "spell": "guiyanei",
          "state": "南美洲",
          "french": "Guyane",
          "italian": "Guyana",
          "spanish": "Guyana",
          "japanese": "ガイアナ",
          "russian": "Гайана",
          "germen": "Guyana"
        ],
        [
          "abbr": "HT",
          "chinese_sc": "海地",
          "chinese_tc": "海地",
          "code": "509",
          "english": "Haiti",
          "spell": "haidi",
          "state": "北美洲",
          "french": "Haïti",
          "italian": "Haiti",
          "spanish": "Haiti",
          "japanese": "ハイチ",
          "russian": "Гаити",
          "germen": "Haiti"
        ],
        [
          "abbr": "HN",
          "chinese_sc": "洪都拉斯",
          "chinese_tc": "洪都拉斯",
          "code": "504",
          "english": "Honduras",
          "spell": "hongdoulasi",
          "state": "北美洲",
          "french": "Honduras",
          "italian": "Honduras",
          "spanish": "Honduras",
          "japanese": "ホンジュラス",
          "russian": "Гондурас",
          "germen": "Honduras"
        ],
        [
          "abbr": "HK",
          "chinese_sc": "中国香港特别行政区",
          "chinese_tc": "中國香港特別行政區",
          "code": "852",
          "english": "Hong Kong SAR China",
          "spell": "zhongguoxianggangtebiexingzhengqu",
          "state": "亚洲",
          "french": "Hong Kong",
          "italian": "Hong Kong",
          "spanish": "Hong Kong",
          "japanese": "香港",
          "russian": "Гонконг SAR Китай",
          "germen": "Hong Kong"
        ],
        [
          "abbr": "HU",
          "chinese_sc": "匈牙利",
          "chinese_tc": "匈牙利",
          "code": "36",
          "english": "Hungary",
          "spell": "xiongyali",
          "state": "欧洲",
          "french": "Hongrie",
          "italian": "Ungheria",
          "spanish": "Hungría",
          "japanese": "ハンガリー",
          "russian": "Венгрия",
          "germen": "Ungarn"
        ],
        [
          "abbr": "IS",
          "chinese_sc": "冰岛",
          "chinese_tc": "冰島",
          "code": "354",
          "english": "Iceland",
          "spell": "bingdao",
          "state": "欧洲",
          "french": "Islande",
          "italian": "Islanda",
          "spanish": "Islandia",
          "japanese": "アイスランド",
          "russian": "Исландия",
          "germen": "Island"
        ],
        [
          "abbr": "IN",
          "chinese_sc": "印度",
          "chinese_tc": "印度",
          "code": "91",
          "english": "India",
          "spell": "yindu",
          "state": "亚洲",
          "french": "Inde",
          "italian": "India",
          "spanish": "India",
          "japanese": "インド",
          "russian": "Индия",
          "germen": "Indien"
        ],
        [
          "abbr": "ID",
          "chinese_sc": "印度尼西亚",
          "chinese_tc": "印度尼西亞",
          "code": "62",
          "english": "Indonesia",
          "spell": "yindunixiya",
          "state": "亚洲",
          "french": "Indonésie",
          "italian": "Indonesia",
          "spanish": "Indonesia",
          "japanese": "インドネシア",
          "russian": "Индонезия",
          "germen": "Indonesien"
        ],
        [
          "abbr": "IR",
          "chinese_sc": "伊朗",
          "chinese_tc": "伊朗",
          "code": "98",
          "english": "Iran",
          "spell": "yilang",
          "state": "亚洲",
          "french": "Iran",
          "italian": "Iran",
          "spanish": "Iran",
          "japanese": "イラン・イスラム共和国",
          "russian": "Иран",
          "germen": "Iran"
        ],
        [
          "abbr": "IQ",
          "chinese_sc": "伊拉克",
          "chinese_tc": "伊拉克",
          "code": "964",
          "english": "Iraq",
          "spell": "yilake",
          "state": "亚洲",
          "french": "Irak",
          "italian": "Iraq",
          "spanish": "Irak",
          "japanese": "イラク",
          "russian": "Ирак",
          "germen": "Irak"
        ],
        [
          "abbr": "IE",
          "chinese_sc": "爱尔兰",
          "chinese_tc": "愛爾蘭",
          "code": "353",
          "english": "Ireland",
          "spell": "aierlan",
          "state": "欧洲",
          "french": "Irlande",
          "italian": "Irlanda",
          "spanish": "Irlanda",
          "japanese": "アイルランド",
          "russian": "Ирландия",
          "germen": "Irland"
        ],
        [
          "abbr": "IM",
          "chinese_sc": "马恩岛",
          "chinese_tc": "馬恩島",
          "code": "44",
          "english": "Isle of Man",
          "spell": "maendao",
          "french": "Royaume-Uni",
          "italian": "Regno Unito",
          "spanish": "Reino Unido",
          "japanese": "イギリス",
          "russian": "Остров Мэн",
          "germen": "Vereinigtes Königreich"
        ],
        [
          "abbr": "IL",
          "chinese_sc": "以色列",
          "chinese_tc": "以色列",
          "code": "972",
          "english": "Israel",
          "spell": "yiselie",
          "state": "亚洲",
          "french": "Israël",
          "italian": "Israele",
          "spanish": "Israel",
          "japanese": "イスラエル",
          "russian": "Израиль",
          "germen": "Israel"
        ],
        [
          "abbr": "IT",
          "chinese_sc": "意大利",
          "chinese_tc": "意大利",
          "code": "39",
          "english": "Italy",
          "spell": "yidali",
          "state": "欧洲",
          "french": "Italie",
          "italian": "Italia",
          "spanish": "Italia",
          "japanese": "イタリア",
          "russian": "Италия",
          "germen": "Italien"
        ],
        [
          "abbr": "CI",
          "chinese_sc": "科特迪瓦",
          "chinese_tc": "科特迪瓦",
          "code": "225",
          "english": "Ivory Coast",
          "spell": "ketediwa",
          "japanese": "コートジボワール",
          "russian": "Кот-д'Ивуар",
          "germen": "Elfenbeinküste"
        ],
        [
          "abbr": "JM",
          "chinese_sc": "牙买加",
          "chinese_tc": "牙買加",
          "code": "1876",
          "english": "Jamaica",
          "spell": "yamaijia",
          "state": "北美洲",
          "french": "Jamaïque",
          "italian": "Giamaica",
          "spanish": "Jamaica",
          "japanese": "ジャマイカ",
          "russian": "Ямайка",
          "germen": "Jamaika"
        ],
        [
          "abbr": "JP",
          "chinese_sc": "日本",
          "chinese_tc": "日本",
          "code": "81",
          "english": "Japan",
          "spell": "riben",
          "state": "亚洲",
          "french": "Japon",
          "italian": "Giappone",
          "spanish": "Japón",
          "japanese": "日本",
          "russian": "Япония",
          "germen": "Japan"
        ],
        [
          "abbr": "JO",
          "chinese_sc": "约旦",
          "chinese_tc": "約旦",
          "code": "962",
          "english": "Jordan",
          "spell": "yuedan",
          "state": "亚洲",
          "french": "Jordanie",
          "italian": "Giordania",
          "spanish": "Jordania",
          "japanese": "ヨルダン",
          "russian": "Иордания",
          "germen": "Jordanien"
        ],
        [
          "abbr": "KZ",
          "chinese_sc": "哈萨克斯坦",
          "chinese_tc": "哈薩克斯坦",
          "code": "7",
          "english": "Kazakstan",
          "spell": "hasakesitan",
          "state": "亚洲",
          "french": "Russie",
          "italian": "Russia",
          "spanish": "Rusia",
          "japanese": "ロシア連邦",
          "russian": "Казахстан",
          "germen": "Russland"
        ],
        [
          "abbr": "KE",
          "chinese_sc": "肯尼亚",
          "chinese_tc": "肯尼亞",
          "code": "254",
          "english": "Kenya",
          "spell": "kenniya",
          "state": "非洲",
          "french": "Kenya",
          "italian": "Kenya",
          "spanish": "Kenia",
          "japanese": "ケニア",
          "russian": "Кения",
          "germen": "Kenia"
        ],
        [
          "abbr": "KR",
          "chinese_sc": "韩国",
          "chinese_tc": "韓國",
          "code": "82",
          "english": "Korea",
          "spell": "hanguo",
          "state": "亚洲",
          "french": "Corée du Sud",
          "italian": "Corea del Sud",
          "spanish": "Corea del Sur",
          "japanese": "大韓民国",
          "russian": "Корея",
          "germen": "Südkorea"
        ],
        [
          "abbr": "KW",
          "chinese_sc": "科威特",
          "chinese_tc": "科威特",
          "code": "965",
          "english": "Kuwait",
          "spell": "keweite",
          "state": "亚洲",
          "french": "Koweït",
          "italian": "Kuwait",
          "spanish": "Kuwait",
          "japanese": "クウェート",
          "russian": "Кувейт",
          "germen": "Kuwait"
        ],
        [
          "abbr": "KG",
          "chinese_sc": "吉尔吉斯斯坦",
          "chinese_tc": "吉爾吉斯斯坦",
          "code": "996",
          "english": "Kyrgyzstan",
          "spell": "jierjisisitan",
          "french": "Kirghizistan",
          "italian": "Kirghizistan",
          "spanish": "Kirguizistán",
          "japanese": "キルギス",
          "russian": "Киргизия",
          "germen": "Kirgisistan"
        ],
        [
          "abbr": "LA",
          "chinese_sc": "老挝",
          "chinese_tc": "老撾",
          "code": "856",
          "english": "Laos",
          "spell": "laowo",
          "state": "亚洲",
          "french": "Laos",
          "italian": "Laos",
          "spanish": "Laos",
          "japanese": "ラオス人民民主共和国",
          "russian": "Лаос",
          "germen": "Laos"
        ],
        [
          "abbr": "LV",
          "chinese_sc": "拉脱维亚",
          "chinese_tc": "拉脫維亞",
          "code": "371",
          "english": "Latvia",
          "spell": "latuoweiya",
          "state": "欧洲",
          "french": "Lettonie",
          "italian": "Lettonia",
          "spanish": "Letonia",
          "japanese": "ラトビア",
          "russian": "Латвия",
          "germen": "Lettland"
        ],
        [
          "abbr": "LB",
          "chinese_sc": "黎巴嫩",
          "chinese_tc": "黎巴嫩",
          "code": "961",
          "english": "Lebanon",
          "spell": "libanen",
          "state": "亚洲",
          "french": "Liban",
          "italian": "Libano",
          "spanish": "Líbano",
          "japanese": "レバノン",
          "russian": "Ливан",
          "germen": "Libanon"
        ],
        [
          "abbr": "LS",
          "chinese_sc": "莱索托",
          "chinese_tc": "萊索托",
          "code": "266",
          "english": "Lesotho",
          "spell": "laisuotuo",
          "french": "Lesotho",
          "italian": "Lesotho",
          "spanish": "Lesotho",
          "japanese": "レソト",
          "russian": "Лесото",
          "germen": "Lesotho"
        ],
        [
          "abbr": "LR",
          "chinese_sc": "利比里亚",
          "chinese_tc": "利比里亞",
          "code": "231",
          "english": "Liberia",
          "spell": "libiliya",
          "french": "Liberia",
          "italian": "Liberia",
          "spanish": "Liberia",
          "japanese": "リベリア",
          "russian": "Либерия",
          "germen": "Liberia"
        ],
        [
          "abbr": "LY",
          "chinese_sc": "利比亚",
          "chinese_tc": "利比亞",
          "code": "218",
          "english": "Libya",
          "spell": "libiya",
          "state": "非洲",
          "french": "Libye",
          "italian": "Libia",
          "spanish": "Libia",
          "japanese": "リビア",
          "russian": "Ливия",
          "germen": "Libyen"
        ],
        [
          "abbr": "LT",
          "chinese_sc": "立陶宛",
          "chinese_tc": "立陶宛",
          "code": "370",
          "english": "Lithuania",
          "spell": "litaowan",
          "state": "欧洲",
          "french": "Lituanie",
          "italian": "Lituania",
          "spanish": "Lituania",
          "japanese": "リトアニア",
          "russian": "Литва",
          "germen": "Litauen"
        ],
        [
          "abbr": "LU",
          "chinese_sc": "卢森堡",
          "chinese_tc": "盧森堡",
          "code": "352",
          "english": "Luxembourg",
          "spell": "lusenbao",
          "state": "亚洲",
          "french": "Luxembourg",
          "italian": "Lussemburgo",
          "spanish": "Luxemburgo",
          "japanese": "ルクセンブルク",
          "russian": "Люксембург",
          "germen": "Luxemburg"
        ],
        [
          "abbr": "MO",
          "chinese_sc": "中国澳门特别行政区",
          "chinese_tc": "中國澳門特別行政區",
          "code": "853",
          "english": "Macao SAR China",
          "spell": "zhongguoaomentebiexingzhengqu",
          "state": "亚洲",
          "french": "Macao",
          "italian": "Macao",
          "spanish": "Macao",
          "japanese": "マカオ",
          "russian": "Макао SAR Китай",
          "germen": "Macao"
        ],
        [
          "abbr": "MK",
          "chinese_sc": "马其顿",
          "chinese_tc": "馬其頓",
          "code": "389",
          "english": "Macedonia",
          "spell": "maqidun",
          "french": "Macédoine",
          "italian": "Macedonia",
          "spanish": "Macedonia",
          "japanese": "マケドニア旧ユーゴスラビア共和国",
          "russian": "Македония",
          "germen": "Mazedonien"
        ],
        [
          "abbr": "MG",
          "chinese_sc": "马达加斯加",
          "chinese_tc": "馬達加斯加",
          "code": "261",
          "english": "Madagascar",
          "spell": "madajiasijia",
          "state": "非洲",
          "french": "Madagascar",
          "italian": "Madagascar",
          "spanish": "Madagascar",
          "japanese": "マダガスカル",
          "russian": "Мадагаскар",
          "germen": "Madagaskar"
        ],
        [
          "abbr": "MW",
          "chinese_sc": "马拉维",
          "chinese_tc": "馬拉維",
          "code": "265",
          "english": "Malawi",
          "spell": "malawei",
          "french": "Malawi",
          "italian": "Malawi",
          "spanish": "Malawi",
          "japanese": "マラウイ",
          "russian": "Малави",
          "germen": "Malawi"
        ],
        [
          "abbr": "MY",
          "chinese_sc": "马来西亚",
          "chinese_tc": "馬來西亞",
          "code": "60",
          "english": "Malaysia",
          "spell": "malaixiya",
          "state": "亚洲",
          "french": "Malaisie",
          "italian": "Malesia",
          "spanish": "Malasia",
          "japanese": "マレーシア",
          "russian": "Малайзия",
          "germen": "Malaysia"
        ],
        [
          "abbr": "MV",
          "chinese_sc": "马尔代夫",
          "chinese_tc": "馬爾代夫",
          "code": "960",
          "english": "Maldives",
          "spell": "maerdaifu",
          "state": "亚洲",
          "french": "Maldives",
          "italian": "Maldive",
          "spanish": "Maldivas",
          "japanese": "モルディブ",
          "russian": "Мальдивы",
          "germen": "Malediven"
        ],
        [
          "abbr": "ML",
          "chinese_sc": "马里",
          "chinese_tc": "馬里",
          "code": "223",
          "english": "Mali",
          "spell": "mali",
          "state": "非洲",
          "french": "Mali",
          "italian": "Mali",
          "spanish": "Mali",
          "japanese": "マリ",
          "russian": "Мали",
          "germen": "Mali"
        ],
        [
          "abbr": "MT",
          "chinese_sc": "马耳他",
          "chinese_tc": "馬耳他",
          "code": "356",
          "english": "Malta",
          "spell": "maerta",
          "state": "欧洲",
          "french": "Malte",
          "italian": "Malta",
          "spanish": "Malta",
          "japanese": "マルタ",
          "russian": "Мальта",
          "germen": "Malta"
        ],
        [
          "abbr": "MR",
          "chinese_sc": "毛利塔尼亚",
          "chinese_tc": "毛利塔尼亞",
          "code": "222",
          "english": "Mauritania",
          "spell": "maolitaniya",
          "state": "非洲",
          "french": "Mauritanie",
          "italian": "Mauritania",
          "spanish": "Mauritania",
          "japanese": "モーリタニア",
          "russian": "Мавритания",
          "germen": "Mauretanien"
        ],
        [
          "abbr": "MU",
          "chinese_sc": "毛里求斯",
          "chinese_tc": "毛里求斯",
          "code": "230",
          "english": "Mauritius",
          "spell": "maoliqiusi",
          "state": "非洲",
          "french": "Île Maurice",
          "italian": "Mauritius",
          "spanish": "Mauricio",
          "japanese": "モーリシャス",
          "russian": "Маврикий",
          "germen": "Mauritius"
        ],
        [
          "abbr": "MX",
          "chinese_sc": "墨西哥",
          "chinese_tc": "墨西哥",
          "code": "52",
          "english": "Mexico",
          "spell": "moxige",
          "state": "北美洲",
          "french": "Mexique",
          "italian": "Messico",
          "spanish": "México",
          "japanese": "メキシコ",
          "russian": "Мексика",
          "germen": "Mexiko"
        ],
        [
          "abbr": "MD",
          "chinese_sc": "摩尔多瓦",
          "chinese_tc": "摩爾多瓦",
          "code": "373",
          "english": "Moldova",
          "spell": "moerduowa",
          "state": "欧洲",
          "french": "Moldavie",
          "italian": "Moldavia",
          "spanish": "Moldavia",
          "japanese": "モルドバ共和国",
          "russian": "Молдавия",
          "germen": "Moldawie"
        ],
        [
          "abbr": "MC",
          "chinese_sc": "摩纳哥",
          "chinese_tc": "摩納哥",
          "code": "377",
          "english": "Monaco",
          "spell": "monage",
          "state": "欧洲",
          "french": "Monaco",
          "italian": "Principato di Monaco",
          "spanish": "Mónaco",
          "japanese": "モナコ",
          "russian": "Монако",
          "germen": "Monaco"
        ],
        [
          "abbr": "MN",
          "chinese_sc": "蒙古",
          "chinese_tc": "蒙古",
          "code": "976",
          "english": "Mongolia",
          "spell": "menggu",
          "state": "亚洲",
          "french": "Mongolie",
          "italian": "Mongolia",
          "spanish": "Mongolia",
          "japanese": "モンゴル",
          "russian": "Монголия",
          "germen": "Mongolei"
        ],
        [
          "abbr": "ME",
          "chinese_sc": "黑山共和国",
          "chinese_tc": "黑山共和國",
          "code": "382",
          "english": "Montenegro",
          "spell": "heishangongheguo",
          "french": "Monténégro",
          "italian": "Montenegro",
          "spanish": "Montenegro",
          "japanese": "モンテネグロ",
          "russian": "Черногория",
          "germen": "Montenegro"
        ],
        [
          "abbr": "MA",
          "chinese_sc": "摩洛哥",
          "chinese_tc": "摩洛哥",
          "code": "212",
          "english": "Morocco",
          "spell": "moluoge",
          "state": "非洲",
          "french": "Sahara Occidental",
          "italian": "Sahara Occidentale",
          "spanish": "Sahara Occidental",
          "japanese": "西サハラ",
          "russian": "Марокко",
          "germen": "Westsahara"
        ],
        [
          "abbr": "MZ",
          "chinese_sc": "莫桑比克",
          "chinese_tc": "莫桑比克",
          "code": "258",
          "english": "Mozambique",
          "spell": "mosangbike",
          "state": "非洲",
          "french": "Mozambique",
          "italian": "Mozambico",
          "spanish": "Mozambique",
          "japanese": "モザンビーク",
          "russian": "Мозамбик",
          "germen": "Mosambik"
        ],
        [
          "abbr": "MM",
          "chinese_sc": "缅甸",
          "chinese_tc": "緬甸",
          "code": "95",
          "english": "Myanmar(Burma)",
          "spell": "miandian",
          "french": "Myanmar",
          "italian": "Birmania",
          "spanish": "Myanmar",
          "japanese": "ミャンマー",
          "russian": "Мьянма (Бирма)",
          "germen": "Myanmar"
        ],
        [
          "abbr": "NA",
          "chinese_sc": "纳米比亚",
          "chinese_tc": "納米比亞",
          "code": "264",
          "english": "Namibia",
          "spell": "namibiya",
          "state": "非洲",
          "french": "Namibie",
          "italian": "Namibia",
          "spanish": "Namibia",
          "japanese": "ナミビア",
          "russian": "Намибия",
          "germen": "Namibia"
        ],
        [
          "abbr": "NP",
          "chinese_sc": "尼泊尔",
          "chinese_tc": "尼泊爾",
          "code": "977",
          "english": "Nepal",
          "spell": "niboer",
          "state": "亚洲",
          "french": "Népal",
          "italian": "Nepal",
          "spanish": "Nepal",
          "japanese": "ネパール",
          "russian": "Непал",
          "germen": "Népal"
        ],
        [
          "abbr": "NL",
          "chinese_sc": "荷兰",
          "chinese_tc": "荷蘭",
          "code": "31",
          "english": "Netherlands",
          "spell": "helan",
          "state": "欧洲",
          "french": "Pays-Bas",
          "italian": "Paesi Bassi",
          "spanish": "Países Bajos",
          "japanese": "オランダ",
          "russian": "Нидерланды",
          "germen": "Niederlande"
        ],
        [
          "abbr": "NZ",
          "chinese_sc": "新西兰",
          "chinese_tc": "新西蘭",
          "code": "64",
          "english": "New Zealand",
          "spell": "xinxilan",
          "state": "大洋洲",
          "french": "Îles Pitcairn",
          "italian": "Isole Pitcairn",
          "spanish": "Islas Pitcairn",
          "japanese": "ピトケアン",
          "russian": "Новая Зеландия",
          "germen": "Pitcairn"
        ],
        [
          "abbr": "NI",
          "chinese_sc": "尼加拉瓜",
          "chinese_tc": "尼加拉瓜",
          "code": "505",
          "english": "Nicaragua",
          "spell": "nijialagua",
          "state": "北美洲",
          "french": "Nicaragua",
          "italian": "Nicaragua",
          "spanish": "Nicaragua",
          "japanese": "ニカラグア",
          "russian": "Никарагуа",
          "germen": "Nicaragua"
        ],
        [
          "abbr": "NE",
          "chinese_sc": "尼日尔",
          "chinese_tc": "尼日爾",
          "code": "227",
          "english": "Niger",
          "spell": "nirier",
          "state": "非洲",
          "french": "Niger",
          "italian": "Niger",
          "spanish": "Níger",
          "japanese": "ニジェール",
          "russian": "Нигер",
          "germen": "Niger"
        ],
        [
          "abbr": "NG",
          "chinese_sc": "尼日利亚",
          "chinese_tc": "尼日利亞",
          "code": "234",
          "english": "Nigeria",
          "spell": "niriliya",
          "state": "非洲",
          "french": "Nigéria",
          "italian": "Nigeria",
          "spanish": "Nigeria",
          "japanese": "ナイジェリア",
          "russian": "Нигерия",
          "germen": "Nigeria"
        ],
        [
          "abbr": "KP",
          "chinese_sc": "朝鲜",
          "chinese_tc": "朝鮮",
          "code": "850",
          "english": "North Korea",
          "spell": "chaoxian",
          "state": "亚洲",
          "french": "Corée du Nord",
          "italian": "Corea del Nord",
          "spanish": "Corea del Norte",
          "japanese": "朝鮮民主主義人民共和国",
          "russian": "Северная Корея",
          "germen": "Nordkorea"
        ],
        [
          "abbr": "NO",
          "chinese_sc": "挪威",
          "chinese_tc": "挪威",
          "code": "47",
          "english": "Norway",
          "spell": "nuowei",
          "state": "欧洲",
          "french": "Norvège",
          "italian": "Norvegia",
          "spanish": "Noruega",
          "japanese": "ノルウェー",
          "russian": "Норвегия",
          "germen": "Norwegen"
        ],
        [
          "abbr": "OM",
          "chinese_sc": "阿曼",
          "chinese_tc": "阿曼",
          "code": "968",
          "english": "Oman",
          "spell": "aman",
          "state": "亚洲",
          "french": "Oman",
          "italian": "oman",
          "spanish": "Omán",
          "japanese": "オマーン",
          "russian": "Оман",
          "germen": "Oman"
        ],
        [
          "abbr": "PK",
          "chinese_sc": "巴基斯坦",
          "chinese_tc": "巴基斯坦",
          "code": "92",
          "english": "Pakistan",
          "spell": "bajisitan",
          "state": "亚洲",
          "french": "Pakistan",
          "italian": "Pakistan",
          "spanish": "Pakistán",
          "japanese": "パキスタン",
          "russian": "Пакистан",
          "germen": "Pakistan"
        ],
        [
          "abbr": "PA",
          "chinese_sc": "巴拿马",
          "chinese_tc": "巴拿馬",
          "code": "507",
          "english": "Panama",
          "spell": "banama",
          "state": "北美洲",
          "french": "Panama",
          "italian": "Panama",
          "spanish": "Panamá",
          "japanese": "パナマ",
          "russian": "Панама",
          "germen": "Panama"
        ],
        [
          "abbr": "PY",
          "chinese_sc": "巴拉圭",
          "chinese_tc": "巴拉圭",
          "code": "595",
          "english": "Paraguay",
          "spell": "balagui",
          "state": "南美洲",
          "french": "Paraguay",
          "italian": "Paraguay",
          "spanish": "Paraguay",
          "japanese": "パラグアイ",
          "russian": "Парагвай",
          "germen": "Paraguay"
        ],
        [
          "abbr": "PE",
          "chinese_sc": "秘鲁",
          "chinese_tc": "秘魯",
          "code": "51",
          "english": "Peru",
          "spell": "milu",
          "state": "南美洲",
          "french": "Pérou",
          "italian": "Perù",
          "spanish": "Perú",
          "japanese": "ペルー",
          "russian": "Перу",
          "germen": "Peru"
        ],
        [
          "abbr": "PH",
          "chinese_sc": "菲律宾",
          "chinese_tc": "菲律賓",
          "code": "63",
          "english": "Philippines",
          "spell": "feilvbin",
          "state": "亚洲",
          "french": "Philippines",
          "italian": "Filippine",
          "spanish": "Filipinas",
          "japanese": "フィリピン",
          "russian": "Филиппины",
          "germen": "Philippinen"
        ],
        [
          "abbr": "PL",
          "chinese_sc": "波兰",
          "chinese_tc": "波蘭",
          "code": "48",
          "english": "Poland",
          "spell": "bolan",
          "state": "欧洲",
          "french": "Pologne",
          "italian": "Polonia",
          "spanish": "Polonia",
          "japanese": "ポーランド",
          "russian": "Польша",
          "germen": "Polen"
        ],
        [
          "abbr": "PT",
          "chinese_sc": "葡萄牙",
          "chinese_tc": "葡萄牙",
          "code": "351",
          "english": "Portugal",
          "spell": "putaoya",
          "state": "欧洲",
          "french": "Portugal",
          "italian": "Portogallo",
          "spanish": "Portugal",
          "japanese": "ポルトガル",
          "russian": "Португалия",
          "germen": "Portugal"
        ],
        [
          "abbr": "PR",
          "chinese_sc": "波多黎各",
          "chinese_tc": "波多黎各",
          "code": "1787",
          "english": "Puerto Rico",
          "spell": "boduolige",
          "french": "Porto Rico",
          "italian": "Porto Rico",
          "spanish": "Puerto Rico",
          "japanese": "プエルトリコ",
          "russian": "Пуэрто-Рико",
          "germen": "Puerto Rico"
        ],
        [
          "abbr": "QA",
          "chinese_sc": "卡塔尔",
          "chinese_tc": "卡塔爾",
          "code": "974",
          "english": "Qatar",
          "spell": "kataer",
          "state": "亚洲",
          "french": "Qatar",
          "italian": "Qatar",
          "spanish": "Catar",
          "japanese": "カタール",
          "russian": "Катар",
          "germen": "Katar"
        ],
        [
          "abbr": "RE",
          "chinese_sc": "留尼旺",
          "chinese_tc": "留尼旺",
          "code": "262",
          "english": "Reunion",
          "spell": "liuniwang",
          "french": "Réunion",
          "italian": "Riunione",
          "spanish": "Reunión",
          "japanese": "レユニオン",
          "russian": "воссоединение",
          "germen": "Réunion"
        ],
        [
          "abbr": "RO",
          "chinese_sc": "罗马尼亚",
          "chinese_tc": "羅馬尼亞",
          "code": "40",
          "english": "Romania",
          "spell": "luomaniya",
          "state": "欧洲",
          "french": "Roumanie",
          "italian": "Romania",
          "spanish": "Rumania",
          "japanese": "ルーマニア",
          "russian": "Румыния",
          "germen": "Rumänien"
        ],
        [
          "abbr": "RU",
          "chinese_sc": "俄罗斯",
          "chinese_tc": "俄羅斯",
          "code": "7",
          "english": "Russia",
          "spell": "eluosi",
          "state": "欧洲",
          "french": "Russie",
          "italian": "Russia",
          "spanish": "Rusia",
          "japanese": "ロシア連邦",
          "russian": "Россия",
          "germen": "Russland"
        ],
        [
          "abbr": "RW",
          "chinese_sc": "卢旺达",
          "chinese_tc": "盧旺達",
          "code": "250",
          "english": "Rwanda",
          "spell": "luwangda",
          "state": "非洲",
          "french": "Rwanda",
          "italian": "Ruanda",
          "spanish": "Ruanda",
          "japanese": "ルワンダ",
          "russian": "Руанда",
          "germen": "Ruanda"
        ],
        [
          "abbr": "SM",
          "chinese_sc": "圣马力诺",
          "chinese_tc": "聖馬力諾",
          "code": "378",
          "english": "San Marino",
          "spell": "shengmalinuo",
          "state": "欧洲",
          "french": "Saint-Marin",
          "italian": "San Marino",
          "spanish": "San Marino",
          "japanese": "サンマリノ",
          "russian": "Сан-Марино",
          "germen": "San Marino"
        ],
        [
          "abbr": "SA",
          "chinese_sc": "沙特阿拉伯",
          "chinese_tc": "沙特阿拉伯",
          "code": "966",
          "english": "Saudi Arabia",
          "spell": "shatealabo",
          "state": "亚洲",
          "french": "Arabie Saoudite",
          "italian": "Arabia Saudita",
          "spanish": "Arabia Saudí",
          "japanese": "サウジアラビア",
          "russian": "Саудовская Аравия",
          "germen": "Saudi-Arabien"
        ],
        [
          "abbr": "SN",
          "chinese_sc": "塞内加尔",
          "chinese_tc": "塞內加爾",
          "code": "221",
          "english": "Senegal",
          "spell": "saineijiaer",
          "state": "非洲",
          "french": "Sénégal",
          "italian": "Senegal",
          "spanish": "Senegal",
          "japanese": "セネガル",
          "russian": "Сенегал",
          "germen": "Senegal"
        ],
        [
          "abbr": "RS",
          "chinese_sc": "塞尔维亚",
          "chinese_tc": "塞爾維亞",
          "code": "381",
          "english": "Serbia",
          "spell": "saierweiya",
          "french": "Serbie",
          "italian": "Serbia",
          "spanish": "Serbia",
          "japanese": "セルビア",
          "russian": "Сербия",
          "germen": "Serbien"
        ],
        [
          "abbr": "SL",
          "chinese_sc": "塞拉利昂",
          "chinese_tc": "塞拉利昂",
          "code": "232",
          "english": "Sierra Leone",
          "spell": "sailaliang",
          "french": "Sierra Leone",
          "italian": "Sierra Leone",
          "spanish": "Sierra Leone",
          "japanese": "シエラレオネ",
          "russian": "Сьерра-Леоне",
          "germen": "Sierra Leone"
        ],
        [
          "abbr": "SG",
          "chinese_sc": "新加坡",
          "chinese_tc": "新加坡",
          "code": "65",
          "english": "Singapore",
          "spell": "xinjiapo",
          "state": "亚洲",
          "french": "Singapour",
          "italian": "Singapore",
          "spanish": "Singapur",
          "japanese": "シンガポール",
          "russian": "Сингапур",
          "germen": "Singapur"
        ],
        [
          "abbr": "SK",
          "chinese_sc": "斯洛伐克",
          "chinese_tc": "斯洛伐克",
          "code": "421",
          "english": "Slovakia",
          "spell": "siluofake",
          "state": "欧洲",
          "french": "Slovaquie",
          "italian": "Slovacchia",
          "spanish": "República Eslovaca",
          "japanese": "スロバキア",
          "russian": "Словакия",
          "germen": "Slowakei"
        ],
        [
          "abbr": "SI",
          "chinese_sc": "斯洛文尼亚",
          "chinese_tc": "斯洛文尼亞",
          "code": "386",
          "english": "Slovenia",
          "spell": "siluowenniya",
          "french": "Slovénie",
          "italian": "Slovenia",
          "spanish": "Eslovenia",
          "japanese": "スロベニア",
          "russian": "Словения",
          "germen": "Slowenien"
        ],
        [
          "abbr": "SO",
          "chinese_sc": "索马里",
          "chinese_tc": "索馬里",
          "code": "252",
          "english": "Somalia",
          "spell": "suomali",
          "state": "非洲",
          "french": "Somalie",
          "italian": "Somalia",
          "spanish": "Somalia",
          "japanese": "ソマリア",
          "russian": "Сомали",
          "germen": "Somalia"
        ],
        [
          "abbr": "ZA",
          "chinese_sc": "南非",
          "chinese_tc": "南非",
          "code": "27",
          "english": "South Africa",
          "spell": "nanfei",
          "state": "非洲",
          "french": "Afrique du Sud",
          "italian": "Sud Africa",
          "spanish": "República de Sudáfrica",
          "japanese": "南アフリカ",
          "russian": "Южно-Африканская Республика (ЮАР)",
          "germen": "Republik Südafrika"
        ],
        [
          "abbr": "ES",
          "chinese_sc": "西班牙",
          "chinese_tc": "西班牙",
          "code": "34",
          "english": "Spain",
          "spell": "xibanya",
          "state": "欧洲",
          "french": "Espagne",
          "italian": "Spagna",
          "spanish": "España",
          "japanese": "スペイン",
          "russian": "Испания",
          "germen": "Spanien"
        ],
        [
          "abbr": "LK",
          "chinese_sc": "斯里兰卡",
          "chinese_tc": "斯里蘭卡",
          "code": "94",
          "english": "Sri Lanka",
          "spell": "sililanka",
          "french": "Sri Lanka",
          "italian": "Sri Lanka",
          "spanish": "Sri Lanka",
          "japanese": "スリランカ",
          "russian": "Шри-Ланка",
          "germen": "Sri Lanka"
        ],
        [
          "abbr": "SD",
          "chinese_sc": "苏丹",
          "chinese_tc": "蘇丹",
          "code": "249",
          "english": "Sudan",
          "spell": "sudan",
          "state": "非洲",
          "french": "Soudan",
          "italian": "Sudan",
          "spanish": "Sudán",
          "japanese": "スーダン",
          "russian": "Судан",
          "germen": "Sudan"
        ],
        [
          "abbr": "SR",
          "chinese_sc": "苏里南",
          "chinese_tc": "蘇里南",
          "code": "597",
          "english": "Suriname",
          "spell": "sulinan",
          "french": "Surinam",
          "italian": "Suriname",
          "spanish": "Surinam",
          "japanese": "スリナム",
          "russian": "Суринам",
          "germen": "Suriname"
        ],
        [
          "abbr": "SZ",
          "chinese_sc": "斯威士兰",
          "chinese_tc": "斯威士蘭",
          "code": "268",
          "english": "Swaziland",
          "spell": "siweishilan",
          "french": "Swaziland",
          "italian": "Swaziland",
          "spanish": "Suazilandia",
          "japanese": "スワジランド",
          "russian": "Свазиленд",
          "germen": "Swasiland"
        ],
        [
          "abbr": "SE",
          "chinese_sc": "瑞典",
          "chinese_tc": "瑞典",
          "code": "46",
          "english": "Sweden",
          "spell": "ruidian",
          "state": "欧洲",
          "french": "Suède",
          "italian": "Svezia",
          "spanish": "Suecia",
          "japanese": "スウェーデン",
          "russian": "Швеция",
          "germen": "Schweden"
        ],
        [
          "abbr": "CH",
          "chinese_sc": "瑞士",
          "chinese_tc": "瑞士",
          "code": "41",
          "english": "Switzerland",
          "spell": "ruishi",
          "state": "欧洲",
          "french": "Suisse",
          "italian": "Svizzera",
          "spanish": "Suiza",
          "japanese": "スイス",
          "russian": "Швейцария",
          "germen": "Schweiz"
        ],
        [
          "abbr": "SY",
          "chinese_sc": "叙利亚",
          "chinese_tc": "敘利亞",
          "code": "963",
          "english": "Syria",
          "spell": "xuliya",
          "state": "亚洲",
          "french": "Syrie",
          "italian": "Siria",
          "spanish": "Siria",
          "japanese": "シリア・アラブ共和国",
          "russian": "Сирия",
          "germen": "Syrien"
        ],
        [
          "abbr": "TW",
          "chinese_sc": "中国台湾",
          "chinese_tc": "中國台灣",
          "code": "886",
          "english": "Taiwan",
          "spell": "zhongguotaiwan",
          "state": "亚洲",
          "french": "Taïwan",
          "italian": "Taiwan",
          "spanish": "Taiwán",
          "japanese": "台湾（中華民国）",
          "russian": "Тайвань",
          "germen": "Taiwan"
        ],
        [
          "abbr": "TJ",
          "chinese_sc": "塔吉克斯坦",
          "chinese_tc": "塔吉克斯坦",
          "code": "992",
          "english": "Tajikstan",
          "spell": "tajikesitan",
          "state": "亚洲",
          "french": "Tadjikistan",
          "italian": "Tagikistan",
          "spanish": "Tayikistán",
          "japanese": "タジキスタン",
          "russian": "Таджикистан",
          "germen": "Tadschikistan"
        ],
        [
          "abbr": "TZ",
          "chinese_sc": "坦桑尼亚",
          "chinese_tc": "坦桑尼亞",
          "code": "255",
          "english": "Tanzania",
          "spell": "tansangniya",
          "state": "非洲",
          "french": "Tanzanie",
          "italian": "Tanzania",
          "spanish": "Tanzania",
          "japanese": "タンザニア",
          "russian": "Танзания",
          "germen": "Tansania"
        ],
        [
          "abbr": "TH",
          "chinese_sc": "泰国",
          "chinese_tc": "泰國",
          "code": "66",
          "english": "Thailand",
          "spell": "taiguo",
          "state": "亚洲",
          "french": "Thaïlande",
          "italian": "Tailandia",
          "spanish": "Tailandia",
          "japanese": "タイ",
          "russian": "Таиланд",
          "germen": "Thailand"
        ],
        [
          "abbr": "TG",
          "chinese_sc": "多哥",
          "chinese_tc": "多哥",
          "code": "228",
          "english": "Togo",
          "spell": "duoge",
          "french": "Togo",
          "italian": "Togo",
          "spanish": "Togo",
          "japanese": "トーゴ",
          "russian": "Того",
          "germen": "Togo"
        ],
        [
          "abbr": "TO",
          "chinese_sc": "汤加",
          "chinese_tc": "湯加",
          "code": "676",
          "english": "Tonga",
          "spell": "tangjia",
          "state": "大洋洲",
          "french": "Tonga",
          "italian": "Tonga",
          "spanish": "Tonga",
          "japanese": "トンガ",
          "russian": "Тонга",
          "germen": "Tonga"
        ],
        [
          "abbr": "TT",
          "chinese_sc": "特立尼达和多巴哥",
          "chinese_tc": "特立尼達和多巴哥",
          "code": "1868",
          "english": "Trinidad and Tobago",
          "spell": "telinidaheduobage",
          "french": "Trinité et Tobago",
          "italian": "Trinidad e Tobago",
          "spanish": "Trinidad y Tobago",
          "japanese": "トリニダード・トバゴ",
          "russian": "Тринидад и Тобаго",
          "germen": "Trinidad und Tobago"
        ],
        [
          "abbr": "TN",
          "chinese_sc": "突尼斯",
          "chinese_tc": "突尼斯",
          "code": "216",
          "english": "Tunisia",
          "spell": "tunisi",
          "state": "非洲",
          "french": "Tunisie",
          "italian": "Tunisia",
          "spanish": "Túnez",
          "japanese": "チュニジア",
          "russian": "Тунис",
          "germen": "Tunesien"
        ],
        [
          "abbr": "TR",
          "chinese_sc": "土耳其",
          "chinese_tc": "土耳其",
          "code": "90",
          "english": "Turkey",
          "spell": "tuerqi",
          "italian": "Santa Sede",
          "spanish": "Santa Sede",
          "japanese": "聖座",
          "russian": "Ватикан",
          "germen": "Heiliger Stuhl"
        ],
        [
          "abbr": "VE",
          "chinese_sc": "委内瑞拉",
          "chinese_tc": "委內瑞拉",
          "code": "58",
          "english": "Venezuela",
          "spell": "weineiruila",
          "state": "北美洲",
          "french": "Venezuela",
          "italian": "Venezuela",
          "spanish": "Venezuela",
          "japanese": "ベネズエラ・ボリバル共和国",
          "russian": "Венесуэла",
          "germen": "Venezuela"
        ],
        [
          "abbr": "VN",
          "chinese_sc": "越南",
          "chinese_tc": "越南",
          "code": "84",
          "english": "Vietnam",
          "spell": "yuenan",
          "state": "亚洲",
          "french": "Viêt Nam",
          "italian": "Vietnam",
          "spanish": "Vietnam",
          "japanese": "ベトナム",
          "russian": "Вьетнам",
          "germen": "Vietnam"
        ],
        [
          "abbr": "US",
          "chinese_sc": "美国",
          "chinese_tc": "美國",
          "code": "1",
          "english": "USA",
          "spell": "meiguo",
          "state": "北美洲",
          "french": "États-Unis",
          "spanish": "Estados Unidos",
          "japanese": "アメリカ合衆国",
          "russian": "Соединенные Штаты Америки",
          "germen": "Vereinigte Staaten von Amerika"
        ],
        [
            "abbr": "GB",
            "chinese_sc": "英国",
            "chinese_tc": "英國",
            "code": "44",
            "english": "United Kiongdom",
            "spell": "yingguo",
            "state": "欧洲",
            "french": "Royaume-Uni",
            "italian": "Regno Unito",
            "spanish": "Reino Unido",
            "japanese": "イギリス",
            "russian": "Объединенное Королевство",
            "germen": "Vereinigtes Königreich"
        ],
        [
          "abbr": "YE",
          "chinese_sc": "也门",
          "chinese_tc": "也門",
          "code": "967",
          "english": "Yemen",
          "spell": "yemen",
          "state": "亚洲",
          "french": "Yémen",
          "italian": "Yemen",
          "spanish": "Yemen",
          "japanese": "イエメン",
          "russian": "Йемен",
          "germen": "Jemen"
        ],
        [
          "abbr": "YU",
          "chinese_sc": "南斯拉夫",
          "chinese_tc": "南斯拉夫",
          "code": "381",
          "english": "Yugoslavia",
          "spell": "nansilafu",
          "state": "欧洲",
          "french": "Serbie",
          "italian": "Serbia",
          "spanish": "Serbia",
          "japanese": "セルビア",
          "russian": "Югославия",
          "germen": "Serbien"
        ],
        [
          "abbr": "ZR",
          "chinese_sc": "扎伊尔",
          "chinese_tc": "扎伊爾",
          "code": "243",
          "english": "Zaire",
          "spell": "zhayier",
          "state": "非洲",
          "french": "Congo (Rép. dém.)",
          "italian": "Congo (Rep. Dem.)",
          "spanish": "Congo (Rep. Dem.)",
          "japanese": "コンゴ民主共和国",
          "russian": "Заир",
          "germen": "Kongo (Dem. Rep.)"
        ],
        [
          "abbr": "ZM",
          "chinese_sc": "赞比亚",
          "chinese_tc": "贊比亞",
          "code": "260",
          "english": "Zambia",
          "spell": "zanbiya",
          "state": "非洲",
          "french": "Zambie",
          "italian": "Zambia",
          "spanish": "Zambia",
          "japanese": "ザンビア",
          "russian": "Замбия",
          "germen": "Sambia"
        ],
        [
          "abbr": "ZW",
          "chinese_sc": "津巴布韦",
          "chinese_tc": "津巴布韋",
          "code": "263",
          "english": "Zimbabwe",
          "spell": "jinbabuwei",
          "state": "非洲",
          "french": "Zimbabwe",
          "italian": "Zimbabwe",
          "spanish": "Zimbabue",
          "japanese": "ジンバブエ",
          "russian": "Зимбабве",
          "germen": "Simbabwe"
        ],
      ]
}
